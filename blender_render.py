...
def main():
    ...
    foam_mat_links.new(
        mat.node_tree.nodes["Mix Shader"].outputs[0], mat.node_tree.nodes["Material Output"].inputs["Surface"]
    )
    ...
    mat.node_tree.nodes["Principled BSDF"].inputs[19].default_value = (1.0, 1.0, 1.0, 1.0)
    ...
    mat.node_tree.nodes.new(type="ShaderNodeBsdfTransparent")
    spray_mat_links.new(
        mat.node_tree.nodes["Transparent BSDF"].outputs[0], mat.node_tree.nodes["Mix Shader"].inputs[1]
    )
    # add principled bsdf and connect to the second shader of mix shader
    mat.node_tree.nodes.new(type="ShaderNodeBsdfPrincipled")
    ...
    mat.node_tree.nodes["Musgrave Texture"].inputs[5].default_value = 2
    spray_mat_links.new(
        mat.node_tree.nodes["Musgrave Texture"].outputs[0], mat.node_tree.nodes["Color Ramp"].inputs[0]
    )
    ...
    mat.node_tree.links.new(
        mat.node_tree.nodes["Checker Texture"].outputs[0], mat.node_tree.nodes["Diffuse BSDF"].inputs[0]
    )
    inp = mat.node_tree.nodes["Material Output"].inputs["Surface"]
    ...
    if create_video:
        ...
        for i in range(obj_list_num):
            scene.sequence_editor.sequences.new_image(
                name=obj_name_list_wo_ext[i], filepath=render_img_path[i], channel=1, frame_start=i
            )
